   ...
   +- 'Sort ['i_manager_id ASC NULLS FIRST, 'avg_monthly_sales ASC NULLS FIRST, 'sum_sales ASC NULLS FIRST], true
      +- 'Project [*]
         +- 'Filter (CASE WHEN ('avg_monthly_sales > 0) THEN ('abs(('sum_sales - 'avg_monthly_sales)) / 'avg_monthly_sales) ELSE null END > 0.1)
            +- 'SubqueryAlias `tmp1`
               +- 'Aggregate ['i_manager_id, 'd_moy], ['i_manager_id, 'sum('ss_sales_price) AS sum_sales#0, 'avg('sum('ss_sales_price)) windowspecdefinition('i_manager_id, unspecifiedframe$()) AS avg_monthly_sales#1]
...
GlobalLimit 100
+- LocalLimit 100
   +- Sort [i_manager_id#24 ASC NULLS FIRST, avg_monthly_sales#1 ASC NULLS FIRST, sum_sales#0 ASC NULLS FIRST], true
      +- Project [i_manager_id#24, sum_sales#0, avg_monthly_sales#1]
         +- Filter (CASE WHEN (avg_monthly_sales#1 > cast(0 as double)) THEN (abs((sum_sales#0 - avg_monthly_sales#1)) / avg_monthly_sales#1) ELSE cast(null as double) END > cast(0.1 as double))
            +- SubqueryAlias `tmp1`
               +- Project [i_manager_id#24, sum_sales#0, avg_monthly_sales#1]
                  +- Project [i_manager_id#24, sum_sales#0, _w0#109, avg_monthly_sales#1, avg_monthly_sales#1]
                     +- Window [avg(_w0#109) windowspecdefinition(i_manager_id#24, specifiedwindowframe(RowFrame, unboundedpreceding$(), unboundedfollowing$())) AS avg_monthly_sales#1], [i_manager_id#24]
                        +- Aggregate [i_manager_id#24, d_moy#57], [i_manager_id#24, sum(ss_sales_price#39) AS sum_sales#0, sum(ss_sales_price#39) AS _w0#109]
...
+- LocalLimit 100
   +- Sort [i_manager_id#24 ASC NULLS FIRST, avg_monthly_sales#1 ASC NULLS FIRST, sum_sales#0 ASC NULLS FIRST], true
      +- Project [i_manager_id#24, sum_sales#0, avg_monthly_sales#1]
         +- Filter (CASE WHEN (avg_monthly_sales#1 > 0.0) THEN (abs((sum_sales#0 - avg_monthly_sales#1)) / avg_monthly_sales#1) ELSE null END > 0.1)
            +- Window [avg(_w0#109) windowspecdefinition(i_manager_id#24, specifiedwindowframe(RowFrame, unboundedpreceding$(), unboundedfollowing$())) AS avg_monthly_sales#1], [i_manager_id#24]
               +- Aggregate [i_manager_id#24, d_moy#57], [i_manager_id#24, sum(ss_sales_price#39) AS sum_sales#0, sum(ss_sales_price#39) AS _w0#109]
                  +- Project [i_manager_id#24, ss_sales_price#39, d_moy#57]
                     +- Join Inner, (ss_store_sk#33 = s_store_sk#77)
                        :- Project [i_manager_id#24, ss_store_sk#33, ss_sales_price#39, d_moy#57]
                        :  +- Join Inner, (ss_sold_date_sk#26 = d_date_sk#49)
                        :     :- Project [i_manager_id#24, ss_sold_date_sk#26, ss_store_sk#33, ss_sales_price#39]
                        :     :  +- Join Inner, (ss_item_sk#28 = i_item_sk#4)
                        :     :     :- Project [i_item_sk#4, i_manager_id#24]
...
== Physical Plan ==
TakeOrderedAndProject(limit=100, orderBy=[i_manager_id#24 ASC NULLS FIRST,avg_monthly_sales#1 ASC NULLS FIRST,sum_sales#0 ASC NULLS FIRST], output=[i_manager_id#24,sum_sales#0,avg_monthly_sales#1])
+- *(7) Project [i_manager_id#24, sum_sales#0, avg_monthly_sales#1]
   +- *(7) Filter (CASE WHEN (avg_monthly_sales#1 > 0.0) THEN (abs((sum_sales#0 - avg_monthly_sales#1)) / avg_monthly_sales#1) ELSE null END > 0.1)
      +- Window [avg(_w0#109) windowspecdefinition(i_manager_id#24, specifiedwindowframe(RowFrame, unboundedpreceding$(), unboundedfollowing$())) AS avg_monthly_sales#1], [i_manager_id#24]
         +- *(6) Sort [i_manager_id#24 ASC NULLS FIRST], false, 0
            +- Exchange hashpartitioning(i_manager_id#24, 200)
               +- *(5) HashAggregate(keys=[i_manager_id#24, d_moy#57], functions=[sum(ss_sales_price#39)], output=[i_manager_id#24, sum_sales#0, _w0#109])
                  +- Exchange hashpartitioning(i_manager_id#24, d_moy#57, 200)
                     +- *(4) HashAggregate(keys=[i_manager_id#24, d_moy#57], functions=[partial_sum(ss_sales_price#39)], output=[i_manager_id#24, d_moy#57, sum#111])
                        +- *(4) Project [i_manager_id#24, ss_sales_price#39, d_moy#57]
                           +- *(4) BroadcastHashJoin [ss_store_sk#33], [s_store_sk#77], Inner, BuildRight
                              :- *(4) Project [i_manager_id#24, ss_store_sk#33, ss_sales_price#39, d_moy#57]
                              :  +- *(4) BroadcastHashJoin [ss_sold_date_sk#26], [d_date_sk#49], Inner, BuildRight
                              :     :- *(4) Project [i_manager_id#24, ss_sold_date_sk#26, ss_store_sk#33, ss_sales_price#39]
                              :     :  +- *(4) BroadcastHashJoin [i_item_sk#4], [ss_item_sk#28], Inner, BuildLeft
                              :     :     :- BroadcastExchange HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)))
                              :     :     :  +- *(1) Project [i_item_sk#4, i_manager_id#24]
                              :     :     :     +- *(1) Filter ((((i_category#16 IN (Books,Children,Electronics) && i_class#14 IN (personal,portable,reference,self-help)) && i_brand#12 IN (scholaramalgamalg #14,scholaramalgamalg #7,exportiunivamalg #9,scholaramalgamalg #9)) || ((i_category#16 IN (Women,Music,Men) && i_class#14 IN (accessories,classical,fragrances,pants)) && i_brand#12 IN (amalgimporto #1,edu packscholar #1,exportiimporto #1,importoamalg #1))) && isnotnull(i_item_sk#4))
                              :     :     :        +- *(1) FileScan parquet tpcds.item[i_item_sk#4,i_brand#12,i_class#14,i_category#16,i_manager_id#24] Batched: true, Format: Parquet, Location: InMemoryFileIndex[file:/Users/angela/Desktop/spark-2.4.8-bin-hadoop2.7/spark-warehouse/tpcds.db/i..., PartitionFilters: [], PushedFilters: [Or(And(And(In(i_category, [Books,Children,Electronics]),In(i_class, [personal,portable,reference..., ReadSchema: struct<i_item_sk:int,i_brand:string,i_class:string,i_category:string,i_manager_id:int>
                              ...
                              :              +- *(2) FileScan parquet tpcds.date_dim[d_date_sk#49,d_month_seq#52,d_moy#57] Batched: true, Format: Parquet, Location: InMemoryFileIndex[file:/Users/angela/Desktop/spark-2.4.8-bin-hadoop2.7/spark-warehouse/tpcds.db/d..., PartitionFilters: [], PushedFilters: [In(d_month_seq, [1200,1211,1205,1201,1206,1210,1207,1202,1209,1203,1208,1204]), IsNotNull(d_date..., ReadSchema: struct<d_date_sk:int,d_month_seq:int,d_moy:int>
                              +- BroadcastExchange HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)))
...
Time taken: 4.061 seconds, Fetched 1 row(s)
Error in query: 
