...
Spark master: local[*], Application Id: local-1741580262925
== Parsed Logical Plan ==
'GlobalLimit 100
+- 'LocalLimit 100
   +- 'Sort ['i_category ASC NULLS FIRST, 'i_class ASC NULLS FIRST, 'i_item_id ASC NULLS FIRST, 'i_item_desc ASC NULLS FIRST, 'revenueratio ASC NULLS FIRST], true
      +- 'Aggregate ['i_item_id, 'i_item_desc, 'i_category, 'i_class, 'i_current_price], ['i_item_id, 'i_item_desc, 'i_category, 'i_class, 'i_current_price, 'sum('cs_ext_sales_price) AS itemrevenue#0, (('sum('cs_ext_sales_price) * 100) / 'sum('sum('cs_ext_sales_price)) windowspecdefinition('i_class, unspecifiedframe$())) AS revenueratio#1]
         +- 'Filter ((('cs_item_sk = 'i_item_sk) AND 'i_category IN (Sports,Books,Home)) AND (('cs_sold_date_sk = 'd_date_sk) AND (('d_date >= cast(1999-02-22 as date)) AND ('d_date <= 'date_add(cast(1999-02-22 as date), 30)))))
            +- 'Join Inner
               :- 'Join Inner
      ...
      +- Project [i_item_id#42, i_item_desc#45, i_category#53, i_class#51, i_current_price#46, itemrevenue#0, revenueratio#1]
         +- Project [i_item_id#42, i_item_desc#45, i_category#53, i_class#51, i_current_price#46, itemrevenue#0, _w0#98, _we0#99, ((_w0#98 * cast(100 as double)) / _we0#99) AS revenueratio#1]
            +- Window [sum(_w0#98) windowspecdefinition(i_class#51, specifiedwindowframe(RowFrame, unboundedpreceding$(), unboundedfollowing$())) AS _we0#99], [i_class#51]
               +- Aggregate [i_item_id#42, i_item_desc#45, i_category#53, i_class#51, i_current_price#46], [i_item_id#42, i_item_desc#45, i_category#53, i_class#51, i_current_price#46, sum(cs_ext_sales_price#30) AS itemrevenue#0, sum(cs_ext_sales_price#30) AS _w0#98]
                  +- Filter (((cs_item_sk#22 = i_item_sk#41) AND i_category#53 IN (Sports,Books,Home)) AND ((cs_sold_date_sk#7 = d_date_sk#63) AND ((cast(d_date#65 as date) >= cast(1999-02-22 as date)) AND (cast(d_date#65 as date) <= date_add(cast(1999-02-22 as date), 30)))))
                     +- Join Inner
                        :- Join Inner
...
+- LocalLimit 100
   +- Sort [i_category#53 ASC NULLS FIRST, i_class#51 ASC NULLS FIRST, i_item_id#42 ASC NULLS FIRST, i_item_desc#45 ASC NULLS FIRST, revenueratio#1 ASC NULLS FIRST], true
      +- Project [i_item_id#42, i_item_desc#45, i_category#53, i_class#51, i_current_price#46, itemrevenue#0, ((_w0#98 * 100.0) / _we0#99) AS revenueratio#1]
         +- Window [sum(_w0#98) windowspecdefinition(i_class#51, specifiedwindowframe(RowFrame, unboundedpreceding$(), unboundedfollowing$())) AS _we0#99], [i_class#51]
            +- Aggregate [i_item_id#42, i_item_desc#45, i_category#53, i_class#51, i_current_price#46], [i_item_id#42, i_item_desc#45, i_category#53, i_class#51, i_current_price#46, sum(cs_ext_sales_price#30) AS itemrevenue#0, sum(cs_ext_sales_price#30) AS _w0#98]
               +- Project [cs_ext_sales_price#30, i_item_id#42, i_item_desc#45, i_current_price#46, i_class#51, i_category#53]
                  +- Join Inner, (cs_sold_date_sk#7 = d_date_sk#63)
                     :- Project [cs_sold_date_sk#7, cs_ext_sales_price#30, i_item_id#42, i_item_desc#45, i_current_price#46, i_class#51, i_category#53]
                     :  +- Join Inner, (cs_item_sk#22 = i_item_sk#41)
                     :     :- Project [cs_sold_date_sk#7, cs_item_sk#22, cs_ext_sales_price#30]
                     :     :  +- Filter (isnotnull(cs_item_sk#22) AND isnotnull(cs_sold_date_sk#7))
                     :     :     +- Relation spark_catalog.tpcds.catalog_sales[cs_sold_date_sk#7,cs_sold_time_sk#8,cs_ship_date_sk#9,cs_bill_customer_sk#10,cs_bill_cdemo_sk#11,cs_bill_hdemo_sk#12,cs_bill_addr_sk#13,cs_ship_customer_sk#14,cs_ship_cdemo_sk#15,cs_ship_hdemo_sk#16,cs_ship_addr_sk#17,cs_call_center_sk#18,cs_catalog_page_sk#19,cs_ship_mode_sk#20,cs_warehouse_sk#21,cs_item_sk#22,cs_promo_sk#23,cs_order_number#24,cs_quantity#25,cs_wholesale_cost#26,cs_list_price#27,cs_sales_price#28,cs_ext_discount_amt#29,cs_ext_sales_price#30,... 10 more fields] parquet
      ...
      +- Window [sum(_w0#98) windowspecdefinition(i_class#51, specifiedwindowframe(RowFrame, unboundedpreceding$(), unboundedfollowing$())) AS _we0#99], [i_class#51]
         +- Sort [i_class#51 ASC NULLS FIRST], false, 0
            +- Exchange hashpartitioning(i_class#51, 200), ENSURE_REQUIREMENTS, [plan_id=85]
               +- HashAggregate(keys=[i_item_id#42, i_item_desc#45, i_category#53, i_class#51, i_current_price#46], functions=[sum(cs_ext_sales_price#30)], output=[i_item_id#42, i_item_desc#45, i_category#53, i_class#51, i_current_price#46, itemrevenue#0, _w0#98])
                  +- Exchange hashpartitioning(i_item_id#42, i_item_desc#45, i_category#53, i_class#51, i_current_price#46, 200), ENSURE_REQUIREMENTS, [plan_id=82]
                     +- HashAggregate(keys=[i_item_id#42, i_item_desc#45, i_category#53, i_class#51, knownfloatingpointnormalized(normalizenanandzero(i_current_price#46)) AS i_current_price#46], functions=[partial_sum(cs_ext_sales_price#30)], output=[i_item_id#42, i_item_desc#45, i_category#53, i_class#51, i_current_price#46, sum#101])
                        +- Project [cs_ext_sales_price#30, i_item_id#42, i_item_desc#45, i_current_price#46, i_class#51, i_category#53]
                           +- BroadcastHashJoin [cs_sold_date_sk#7], [d_date_sk#63], Inner, BuildRight, false
                              ...
                              :     :- Filter (isnotnull(cs_item_sk#22) AND isnotnull(cs_sold_date_sk#7))
                              :     :  +- FileScan parquet spark_catalog.tpcds.catalog_sales[cs_sold_date_sk#7,cs_item_sk#22,cs_ext_sales_price#30] Batched: true, DataFilters: [isnotnull(cs_item_sk#22), isnotnull(cs_sold_date_sk#7)], Format: Parquet, Location: InMemoryFileIndex(1 paths)[file:/Users/angela/Desktop/spark-3.5.5-bin-hadoop3/spark-warehouse/tpc..., PartitionFilters: [], PushedFilters: [IsNotNull(cs_item_sk), IsNotNull(cs_sold_date_sk)], ReadSchema: struct<cs_sold_date_sk:int,cs_item_sk:int,cs_ext_sales_price:double>
...
Time taken: 2.411 seconds, Fetched 1 row(s)
