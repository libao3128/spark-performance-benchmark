...
Spark master: local[*], Application Id: local-1741581969285
== Parsed Logical Plan ==
'GlobalLimit 100
+- 'LocalLimit 100
...
GlobalLimit 100
+- LocalLimit 100
   +- Sort [i_manager_id#27 ASC NULLS FIRST, avg_monthly_sales#1 ASC NULLS FIRST, sum_sales#0 ASC NULLS FIRST], true
      +- Project [i_manager_id#27, sum_sales#0, avg_monthly_sales#1]
         +- Filter (CASE WHEN (avg_monthly_sales#1 > cast(0 as double)) THEN (abs((sum_sales#0 - avg_monthly_sales#1)) / avg_monthly_sales#1) ELSE cast(null as double) END > cast(0.1 as double))
            +- SubqueryAlias tmp1
               +- Project [i_manager_id#27, sum_sales#0, avg_monthly_sales#1]
                  +- Project [i_manager_id#27, sum_sales#0, _w0#116, avg_monthly_sales#1, avg_monthly_sales#1]
                     +- Window [avg(_w0#116) windowspecdefinition(i_manager_id#27, specifiedwindowframe(RowFrame, unboundedpreceding$(), unboundedfollowing$())) AS avg_monthly_sales#1], [i_manager_id#27]
                        +- Aggregate [i_manager_id#27, d_moy#60], [i_manager_id#27, sum(ss_sales_price#42) AS sum_sales#0, sum(ss_sales_price#42) AS _w0#116]
...
+- LocalLimit 100
   +- Sort [i_manager_id#27 ASC NULLS FIRST, avg_monthly_sales#1 ASC NULLS FIRST, sum_sales#0 ASC NULLS FIRST], true
      +- Project [i_manager_id#27, sum_sales#0, avg_monthly_sales#1]
         +- Filter CASE WHEN (avg_monthly_sales#1 > 0.0) THEN ((abs((sum_sales#0 - avg_monthly_sales#1)) / avg_monthly_sales#1) > 0.1) ELSE false END
            +- Window [avg(_w0#116) windowspecdefinition(i_manager_id#27, specifiedwindowframe(RowFrame, unboundedpreceding$(), unboundedfollowing$())) AS avg_monthly_sales#1], [i_manager_id#27]
               +- Aggregate [i_manager_id#27, d_moy#60], [i_manager_id#27, sum(ss_sales_price#42) AS sum_sales#0, sum(ss_sales_price#42) AS _w0#116]
                  +- Project [i_manager_id#27, ss_sales_price#42, d_moy#60]
                     +- Join Inner, (ss_store_sk#36 = s_store_sk#80)
                        :- Project [i_manager_id#27, ss_store_sk#36, ss_sales_price#42, d_moy#60]
                        :  +- Join Inner, (ss_sold_date_sk#29 = d_date_sk#52)
                        :     :- Project [i_manager_id#27, ss_sold_date_sk#29, ss_store_sk#36, ss_sales_price#42]
                        :     :  +- Join Inner, (ss_item_sk#31 = i_item_sk#7)
                        :     :     :- Project [i_item_sk#7, i_manager_id#27]
...
+- TakeOrderedAndProject(limit=100, orderBy=[i_manager_id#27 ASC NULLS FIRST,avg_monthly_sales#1 ASC NULLS FIRST,sum_sales#0 ASC NULLS FIRST], output=[i_manager_id#27,sum_sales#0,avg_monthly_sales#1])
   +- Project [i_manager_id#27, sum_sales#0, avg_monthly_sales#1]
      +- Filter CASE WHEN (avg_monthly_sales#1 > 0.0) THEN ((abs((sum_sales#0 - avg_monthly_sales#1)) / avg_monthly_sales#1) > 0.1) ELSE false END
         +- Window [avg(_w0#116) windowspecdefinition(i_manager_id#27, specifiedwindowframe(RowFrame, unboundedpreceding$(), unboundedfollowing$())) AS avg_monthly_sales#1], [i_manager_id#27]
            +- Sort [i_manager_id#27 ASC NULLS FIRST], false, 0
               +- Exchange hashpartitioning(i_manager_id#27, 200), ENSURE_REQUIREMENTS, [plan_id=112]
                  +- HashAggregate(keys=[i_manager_id#27, d_moy#60], functions=[sum(ss_sales_price#42)], output=[i_manager_id#27, sum_sales#0, _w0#116])
                     +- Exchange hashpartitioning(i_manager_id#27, d_moy#60, 200), ENSURE_REQUIREMENTS, [plan_id=109]
                        +- HashAggregate(keys=[i_manager_id#27, d_moy#60], functions=[partial_sum(ss_sales_price#42)], output=[i_manager_id#27, d_moy#60, sum#118])
                           +- Project [i_manager_id#27, ss_sales_price#42, d_moy#60]
                              +- BroadcastHashJoin [ss_store_sk#36], [s_store_sk#80], Inner, BuildRight, false
                                 ...
                                 :     :     :  +- Project [i_item_sk#7, i_manager_id#27]
                                 :     :     :     +- Filter ((((i_category#19 IN (Books,Children,Electronics) AND i_class#17 IN (personal,portable,reference,self-help)) AND i_brand#15 IN (scholaramalgamalg #14,scholaramalgamalg #7,exportiunivamalg #9,scholaramalgamalg #9)) OR ((i_category#19 IN (Women,Music,Men) AND i_class#17 IN (accessories,classical,fragrances,pants)) AND i_brand#15 IN (amalgimporto #1,edu packscholar #1,exportiimporto #1,importoamalg #1))) AND isnotnull(i_item_sk#7))
                                 :     :     :        +- FileScan parquet spark_catalog.tpcds.item[i_item_sk#7,i_brand#15,i_class#17,i_category#19,i_manager_id#27] Batched: true, DataFilters: [(((i_category#19 IN (Books,Children,Electronics) AND i_class#17 IN (personal,portable,reference,..., Format: Parquet, Location: InMemoryFileIndex(1 paths)[file:/Users/angela/Desktop/spark-3.5.5-bin-hadoop3/spark-warehouse/tpc..., PartitionFilters: [], PushedFilters: [Or(And(And(In(i_category, [Books,Children,Electronics]),In(i_class, [personal,portable,reference..., ReadSchema: struct<i_item_sk:int,i_brand:string,i_class:string,i_category:string,i_manager_id:int>
                                 :     :     +- Filter ((isnotnull(ss_item_sk#31) AND isnotnull(ss_sold_date_sk#29)) AND isnotnull(ss_store_sk#36))
